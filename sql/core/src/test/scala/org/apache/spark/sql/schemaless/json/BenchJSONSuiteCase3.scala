...
class BenchJSONSuiteCase3 extends QueryTest with BeforeAndAfterAll {
   ...
   test("JSON-reviews5-non-cached"){

     checkAnswer(
       sql("select userName, rating, gPlusPlaceId, utime " +
         "from GoogleReviews  " +
         "where texttime  = 'Mar 26, 2003' and gPlusUserId='103192135926796949948' "),
       Seq(Row("Tim Shanley",4000.0,"110101154558443724899",1048723200L))
     )
   }
   

   test("JSON-openreviews4-non-cached"){
   ...
   test("JSON-openreviews5-non-cached"){
   ...
   test("JSON-schemaless4-non-cached"){
   ...
   test("JSON-schemaless5-non-cached"){
